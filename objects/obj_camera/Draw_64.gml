...
var _following = (instance_exists(cam1.get_follow())) ? "following" : "not following";
...
draw_text_outline(1, _offset * 8, "[Tab] camera speed " + string(cam1.get_speed()), _outline_width, _precision);
draw_text_outline(1, _offset * 9, "[B] smooth_draw: " + (cam1.get_smooth_draw() ? "ON" : "OFF"), _outline_width, _precision);
...
if(cam1.out_of_bounds(obj_player.x, obj_player.y, 8)){
	...
	_x = clamp(_x, _margin, cam1.get_width() * _gui_scale_x - _margin);
	_y = clamp(_y, _margin, cam1.get_height() * _gui_scale_y - _margin);
